    ...
    return (
        <Paper style={{marginTop: 16, overflowX: 'auto'}}>
            <div style={{display: 'flex', gap: 16, padding: 12, flexWrap: 'wrap'}} className="reports-no-print">
                <FormControl><InputLabel>Seitengröße</InputLabel><Select value={String(pageSize)} onChange={(e) => onPageSize(e.target.value === 'all' ? 'all' : parseInt(e.target.value as string, 10))}>{[25,50,100,250].map((size) => <MenuItem key={size} value={String(size)}>{size}</MenuItem>)}<MenuItem value="all">Alle</MenuItem></Select></FormControl>
                {columns.map((column) => <label key={column.id} style={{display: 'flex', alignItems: 'center'}}><Checkbox checked={column.visible} onChange={(e) => onColumns(columns.map((c) => c.id === column.id ? {...c, visible: e.target.checked} : c))} />{column.label}</label>)}
            </div>
            <Table size="small">
                <TableHead><TableRow>{visibleColumns.map((column) => <TableCell key={column.id} onClick={() => cycleSort(column.id)} style={{cursor: sortable[column.id] ? 'pointer' : 'default', whiteSpace: 'nowrap'}}>{column.label} {sort && sortable[column.id] === sort.key ? (sort.direction === 'asc' ? '↑' : '↓') : ''}</TableCell>)}</TableRow></TableHead>
                <TableBody>{paged.map((entry) => <TableRow key={entry.id}>{visibleColumns.map((column) => <TableCell key={column.id}>{render(entry, column.id)}</TableCell>)}</TableRow>)}</TableBody>
                <TableFooter><TableRow><TableCell colSpan={visibleColumns.length}><Typography variant="subtitle2">Gesamtstunden: {formatDuration(summary.totalMs)} · Gesamteinträge: {summary.count} · Durchschnitt pro Tag: {formatDuration(summary.averagePerDayMs)} · Durchschnitt pro Eintrag: {formatDuration(summary.averagePerEntryMs)} · Längster Eintrag: {formatDuration(summary.longestMs)} · Kürzester Eintrag: {formatDuration(summary.shortestMs)}</Typography></TableCell></TableRow></TableFooter>
            </Table>
            {pageSize !== 'all' ? <div className="reports-no-print" style={{padding: 12}}><button disabled={page === 0} onClick={() => setPage(page - 1)}>Zurück</button> <span>Seite {page + 1} / {Math.max(1, Math.ceil(entries.length / pageSize))}</span> <button disabled={(page + 1) * pageSize >= entries.length} onClick={() => setPage(page + 1)}>Weiter</button></div> : null}
        </Paper>
    );
};
